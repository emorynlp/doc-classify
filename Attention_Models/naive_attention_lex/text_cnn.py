      ...
      embedding_size, filter_sizes, num_filters, embedding_size_lex=2, l2_reg_lambda=0.0):
        ...
        for i, filter_size in enumerate(filter_sizes):
            with tf.name_scope("conv-maxpool-%s" % filter_size):
				...
				# Multiply by learned attention tensor U
                conv_4matmul = tf.reshape(conv, [-1, num_filters], name="conv_4matmul")
                U = tf.Variable(tf.truncated_normal([num_filters, num_filters], stddev=0.1), name="U")
                conv_2dim = tf.matmul(conv_4matmul, U)
                #this seems odd / like it may distort some of the results...
                conv_attn = tf.reshape(conv_2dim, [-1, (sequence_length - filter_size + 1), 1, num_filters])

                # Apply nonlinearity
                h = tf.nn.relu(tf.nn.bias_add(conv_attn, b), name="relu")
